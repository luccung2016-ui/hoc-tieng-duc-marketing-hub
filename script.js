...
(function(){
  document.querySelectorAll('.angle-tabs').forEach(tabWrap=>{
    tabWrap.querySelectorAll('button').forEach(btn=>btn.addEventListener('click',()=>{
      ...
      const target=pillar.querySelector('#'+btn.dataset.target); if(target) target.classList.add('active');
    }));
  });
  const filters=document.querySelectorAll('.objective-tabs button');
  filters.forEach(btn=>btn.addEventListener('click',()=>{
    filters.forEach(b=>b.classList.remove('active')); btn.classList.add('active');
    const obj=btn.dataset.obj;
    document.querySelectorAll('.pillar').forEach(p=>{
      if(obj==='all'){p.style.display='block';return;}
      p.style.display=(p.dataset.objectives||'').split(' ').includes(obj)?'block':'none';
    });
  }));
  const personas={
    duhocnghe:[
